...
export default function Substitution() {
  ...
  const [highlightedPositions, setHighlightedPositions] = useState<number[]>(
    []
  ); // Index of the letters in the standard alphabet that appear in the message
  ...
  useEffect(() => {
    // Creates shuffled alphabet
    const alphabet: Character[] = ALPHABET_ARRAY.map((char) => {
      return { value: char, id: char };
    });
    shuffle(alphabet);

    //Defines the index of the letters in the standard alphabet that appear in the message
    const indexes: number[] = [];
    for (let i = 0; i < ALPHABET_ARRAY.length; i++) {
      if (message.includes(ALPHABET_ARRAY[i])) {
        indexes.push(i);
      }
    }
  ...
  return (
    <div className="flex flex-col w-full h-[calc(100vh-90px)]">
      ...
      <TextualExplanation
        explanation={explanations.cryptography.substitution}
        onClose={() => {
